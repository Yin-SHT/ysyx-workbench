...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = ((sword_t)((char)Mr(src1 + imm, 1))));
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (uint32_t)((((uint64_t)((uint32_t)src1) * (uint64_t)((uint32_t)src2)) >> 32) & 0x00000000FFFFFFFF));
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11100 11", ecall  , I, s->dnpc = isa_raise_intr(R(17), s->pc)); // This should locate after all system instructions
