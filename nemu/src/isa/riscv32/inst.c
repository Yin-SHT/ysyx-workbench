...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // mul-longlong
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (uint32_t)((((int64_t)((int32_t)src1) * (int64_t)((int32_t)src2)) >> 32) & 0x00000000FFFFFFFF););
  // movsx
  // mov-c
  // min3
  // mermence
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh  , R, R(rd) = ((uint32_t)((((int64_t)src1 * (int64_t)src2) >> 32) & 0x00000000FFFFFFFF)));
  // max
  // matrix-mul
  // load-store
  ...
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, R(rd) = s->dnpc = src1 >= src2 ? s->pc + imm : s->dnpc);
  ...
  // For dummy
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = ((src1 + imm) & (~1)), R(rd) = s->snpc; (rd == 0 && BITS(s->isa.inst.val, 19, 15) == 1) ? ftrace_ret(s->pc, s->dnpc) : ftrace_call(s->pc, s->dnpc););
